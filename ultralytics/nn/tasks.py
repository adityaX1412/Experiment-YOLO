...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
                          ...
                          Detect_LSCD, Detect_TADDH, Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Detect_LADH, Segment_LADH, Pose_LADH, OBB_LADH)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
                          ...
                          Detect_LADH, Segment_LADH, Pose_LADH, OBB_LADH)):
            s = 640  # 2x min stride
            ...
            if isinstance(m, (DetectAux,)):
                ...
                forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Pose, Pose_LSCD, Pose_TADDH, OBB, OBB_LSCD, OBB_TADDH, 
                                                                         Segment_LADH, Pose_LADH, OBB_LADH)) else self.forward(x)
            try:
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, 
                 ...
                 OBB, OBB_LSCD, OBB_TADDH, Detect_LADH, Segment_LADH, Pose_LADH, OBB_LADH):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 BottleneckCSP, C1, C2, C2f,ESC2f,SC2f, C3, C3TR, C3Ghost, nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, C2f_Faster, C2f_ODConv,
                 ...
                 C3_Faster_CGLU, C2f_Faster_CGLU,LDConv):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f,ESC2f,SC2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, C2f_DBB,
                     ...
                     C3_FADC, C2f_FADC, C3_PPA, C2f_PPA, RGCSPELAN, C3_Faster_CGLU, C2f_Faster_CGLU):
            ...
            if m in (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Segment_LADH):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, 
                              ...
                              Detect_DyHead_Prune, Detect_LSCD, Detect_TADDH, Detect_LADH):
                return "detect"
            elif isinstance(m, (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Segment_LADH)):
                return "segment"
            elif isinstance(m, Classify):
                return "classify"
            elif isinstance(m, (Pose, Pose_LSCD, Pose_TADDH, Pose_LADH)):
                return "pose"
            elif isinstance(m, (OBB, OBB_LSCD, OBB_TADDH, OBB_LADH)):
                return "obb"
