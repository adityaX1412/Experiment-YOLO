...
def scale_boxes(boxes, pad_x, pad_y, crop_info, ratio):
    if boxes.size == 0:
        return np.array([])
    scaled = boxes.copy()
    scaled[:, 0] -= pad_x; scaled[:, 1] -= pad_y
    scaled[:, 2] -= pad_x; scaled[:, 3] -= pad_y
    scaled[:, 0] /= ratio; scaled[:, 1] /= ratio
    scaled[:, 2] /= ratio; scaled[:, 3] /= ratio
    scaled[:, 0] += crop_info['x1']; scaled[:, 1] += crop_info['y1']
    scaled[:, 2] += crop_info['x1']; scaled[:, 3] += crop_info['y1']
    return scaled
...
def perform_double_inference(image_path, model, original_detection, use_augment=False):
    try:
        start_extra = time.time()  # start timing
        ...
        extra_time = time.time() - start_extra  # elapsed for double inference

        if len(new_results[0].boxes) == 0:
            return None, extra_time
        ...
        refined = process_refined_boxes(scaled_boxes, labels, confs, original_detection, img_width, img_height)
        return refined, extra_time
...
def process_refined_boxes(scaled_boxes, labels, confs, original_detection, img_width, img_height):
    ...
    for box, label, conf in zip(scaled_boxes, labels, confs):
        ...
        if combined_score > best_combined:
            ...
            best_conf = conf
            best_match = {'bbox': box.tolist(), 'score': float(conf), 'category_id': int(label)}
...
def calculate_metrics(predictions, targets):
    ...
    for pred, target in zip(predictions, targets):
        ...
        for box, score, label in zip(pred_boxes, pred_scores, pred_labels):
            ...
            if best_idx >= 0:
                tp += 1; matched_targets.add(best_idx)
            else:
                fp += 1
    ...
    try:
        ...
        for idx in range(len(current_predictions['scores'])):
            if current_predictions['scores'][idx] >= CONF_THRESHOLD:
                ...
                refined, extra_time = perform_double_inference(image_path, model, original_detection, use_augment)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        future_to_image = {
            executor.submit(process_image, image_path, model, image_predictions, model_names, True): image_path
    ...
    logging.info("===== DOUBLE INFERENCE METRICS =====")
