# double_inference_with_visualizations.py
# Full script: performs single-stage and double-stage (refined) inference,
# computes metrics, and saves visualizations to /kaggle/working/visualizations/{gt,single,double}
...
# Visualization root (single root with 3 subfolders)
...
def get_model():
    ...
    if _model_cache is None:
        logging.info("Loading YOLO model weights: %s", MODEL_WEIGHTS)
        _model_cache = YOLO(MODEL_WEIGHTS)
        # Warm up with a dummy PIL image
        dummy = Image.new('RGB', (640, 640), (114, 114, 114))
        try:
            with torch.no_grad():
                _model_cache.predict(dummy, verbose=False)
        except Exception:
            # ignore warmup failures
            logging.info("Model warmup failed (non-fatal)")
    return _model_cache
